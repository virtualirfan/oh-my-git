...
if [ -n "${BASH_VERSION}" ]; then
    ...
    : ${omg_dark_background:='\[\033[48;5;1m\]'}
    ...
    function custom_build_prompt {
        ...
        local black_on_dark="${black}${omg_dark_background}"
        local white_on_dark="${white}${omg_dark_background}"
        ...
        if [[ $is_a_git_repo == true ]]; then
            ...
            prompt="${prompt} ${white_on_dark} ${black_on_dark}"
            if [[ $detached == true ]]; then
                prompt+=$(enrich_append $detached $omg_detached_symbol "${white_on_dark}")
                prompt+=$(enrich_append $detached "(${current_commit_hash:0:7})" "${black_on_dark}")
            else            
                if [[ $has_upstream == false ]]; then
                    prompt+=$(enrich_append true "-- ${omg_not_tracked_branch_symbol}  --  (${current_branch})" "${black_on_dark}")
                else
                    ...
                    if [[ $has_diverged == true ]]; then
                        prompt+=$(enrich_append true "-${commits_behind} ${omg_has_diverged_symbol} +${commits_ahead}" "${white_on_dark}")
                    else
                        if [[ $commits_behind -gt 0 ]]; then
                            prompt+=$(enrich_append true "-${commits_behind} ${white_on_dark}${omg_can_fast_forward_symbol}${black_on_dark} --" "${black_on_dark}")
                        fi
                        if [[ $commits_ahead -gt 0 ]]; then
                            prompt+=$(enrich_append true "-- ${white_on_dark}${omg_should_push_symbol}${black_on_dark}  +${commits_ahead}" "${black_on_dark}")
                        fi
                        if [[ $commits_ahead == 0 && $commits_behind == 0 ]]; then
                            prompt+=$(enrich_append true " --   -- " "${black_on_dark}")
                        fi
                        
                    fi
                    prompt+=$(enrich_append true "(${current_branch} ${type_of_upstream} ${upstream//\/$current_branch/})" "${black_on_dark}")
                fi
            fi
            prompt+=$(enrich_append ${is_on_a_tag} "${omg_is_on_a_tag_symbol} ${tag_at_current_commit}" "${black_on_dark}")
